      ...
      <body>
        <header className="sticky top-0 z-40 border-b border-ink/10 bg-white/95 backdrop-blur">
          <nav className="mx-auto flex max-w-7xl flex-wrap items-center gap-2 px-4 py-3">
            <Link className="focus-ring mr-auto inline-flex items-center gap-3 rounded-md pr-3 text-base font-bold text-ink" href="/">
              <span className="grid size-9 place-items-center rounded-md bg-ink text-white shadow-sm">
                <BarChart3 size={18} />
              </span>
              <span className="leading-tight">{APP_NAME}</span>
            </Link>
            {nav.map(([label, href]) => (
              <Link key={href} className="focus-ring rounded-md px-3 py-2 text-sm font-semibold text-ink/64 transition hover:bg-frost hover:text-ink" href={href}>
                {label}
              </Link>
            ))}
          </nav>
