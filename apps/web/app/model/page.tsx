...
export default async function ModelPage() {
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-3xl font-bold">Model Lab</h1>
      <section className="grid gap-4 lg:grid-cols-3">
        {["accuracy", "log_loss", "brier_score"].map((key) => (
          <div key={key} className="rounded-md border border-ink/10 bg-white/88 p-4">
            <span className="text-sm text-ink/60">{key.replaceAll("_", " ")}</span>
            <strong className="block text-3xl">{String(backtesting[key] ?? "TBD")}</strong>
          </div>
        ))}
      </section>
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-md border border-ink/10 bg-white/88 p-4">
          <h2 className="mb-3 text-lg font-semibold">Feature importance</h2>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {["Calibration chart", "Confusion matrix", "SHAP explanation"].map((label) => <div key={label} className="grid min-h-[180px] place-items-center rounded-md border border-dashed border-ink/25 bg-white/70 text-sm font-semibold text-ink/55">{label}</div>)}
      </section>
    </div>
  );
}
