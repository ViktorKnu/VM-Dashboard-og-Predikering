  ...
  return (
    <div className="space-y-5">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <div className="rounded-md border border-ink/10 bg-white/88 p-4">Scorer først <strong className="block text-3xl">{Math.round(Number(insights.winning_after_scoring_first ?? 0) * 100)}%</strong></div>
        <div className="rounded-md border border-ink/10 bg-white/88 p-4">Leder til pause <strong className="block text-3xl">{Math.round(Number(insights.winning_when_leading_at_halftime ?? 0) * 100)}%</strong></div>
        <div className="rounded-md border border-ink/10 bg-white/88 p-4">Underdog-seier <strong className="block text-3xl">{Math.round(Number(insights.underdog_win_frequency ?? 0) * 100)}%</strong></div>
      </section>
