  ...
  return (
    <div className="space-y-5">
      <section className="rounded-md border border-ink/10 bg-white/88 p-5">
        <div className="text-sm text-ink/60">{match.stage} · {match.stadium}, {match.city}</div>
        <div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-3">
          <TeamBadge team={match.home_team} />
          <div className="rounded-sm bg-ink px-5 py-3 text-2xl font-bold text-white">{match.home_score ?? "-"} : {match.away_score ?? "-"}</div>
          <div className="justify-self-end"><TeamBadge team={match.away_team} /></div>
        </div>
      </section>
        ...
        <div className="space-y-5">
          ...
          <section className="rounded-md border border-ink/10 bg-white/88 p-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="rounded-sm bg-frost p-3">xG hjem <strong className="block">{live.current.home_xg}</strong></div>
              <div className="rounded-sm bg-frost p-3">xG borte <strong className="block">{live.current.away_xg}</strong></div>
              <div className="rounded-sm bg-frost p-3">SOT hjem <strong className="block">{live.current.home_shots_on_target}</strong></div>
              <div className="rounded-sm bg-frost p-3">SOT borte <strong className="block">{live.current.away_shots_on_target}</strong></div>
            </div>
          </section>
        </div>
      </div>
